...
export function DocumentsTable({ onInspect, onRowClick, onDelete, docs }: DocumentsTableProps) {
    ...
    return (
        <>
            ...
            <div className="rounded-2xl border border-border bg-card shadow-[0_2px_8px_rgba(0,0,0,0.04)] overflow-hidden">
                <Table>
                    <TableHeader>
                        <TableRow className="hover:bg-transparent border-border">
                            <TableHead className="w-[25%] text-muted-foreground font-medium">Название</TableHead>
                            <TableHead className="text-muted-foreground font-medium">Тип</TableHead>
                            <TableHead className="text-muted-foreground font-medium">Размер</TableHead>
                            <TableHead className="text-muted-foreground font-medium">Статус</TableHead>
                            <TableHead className="w-[30%] text-muted-foreground font-medium">Описание</TableHead>
                    ...
                    <TableBody>
                        {documents.map((doc) => (
                            <TableRow
                                key={doc.id}
                                className={cn("group cursor-pointer hover:bg-muted/50/80 transition-colors border-border", onRowClick && "cursor-pointer")}
                            ...
                            >
                                <TableCell className="font-medium">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col gap-0.5">
                                            ...
                                            <span className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium">{doc.updatedAt}</span>
                                        </div>
                                    </div>
                                </TableCell>
                                <TableCell>
                                ...
                                <TableCell className="text-muted-foreground text-sm font-mono">{doc.size}</TableCell>
                                <TableCell>
                                        ...
                                        <TooltipProvider>
                                            <Tooltip>
                                                <TooltipTrigger asChild>
                                                    <div
                                                        className="flex items-center gap-2 cursor-pointer hover:bg-muted rounded-lg p-1.5 -m-1.5 transition-colors group/desc"
                                                        onClick={(e) => handleDescriptionClick(e, doc)}
                                                    >
                                                        <span className="text-foreground line-clamp-2 flex-1">
                                                            {doc.aiMetadata.summary}
                                                        </span>
                                                        <Info className="h-4 w-4 text-muted-foreground group-hover/desc:text-muted-foreground flex-shrink-0" />
                                                    </div>
                                                </TooltipTrigger>
